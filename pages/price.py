...
st.set_page_config(page_title="Price & Popularity", layout="wide")

# Sidebar functionality
notes_sidebar()  # Display notes
df = apply_filters_sidebar()  # Apply filters
...
y_categories = ['Games released', 'Average playtime forever', "Peak CCU", 'Reviews', "Review score", 'Recommendations']
...
for i, target_dimension in enumerate(y_categories):
    if target_dimension == "Games released":
        agg_data = df.groupby('Price Bin').size().reset_index(name='Games released')
    else:
        target_df = df[df[target_dimension] > 0] if target_dimension in ("Average playtime forever", "Peak CCU") else df
    ...
    fig = px.bar(agg_data, x='Price Bin', y=target_dimension,
                title=f'Average {target_dimension} by Price Bin',
    ...
    if target_dimension in ("Average playtime forever", "Peak CCU"):
        fig.add_annotation(
